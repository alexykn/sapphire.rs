...
fn apply_manifest_after_removal(manifest: &Manifest, manifest_path: &str) -> Result<()> {
    ...
    if !manifest.taps.is_empty() {
        ...
        let installed_taps = crate::shard::package_processor::get_installed_taps()?;
        for tap in &manifest.taps {
            if !installed_taps.contains(&tap.name) {
                println!("{} Adding tap: {}", 
                    style("→").bold().green(),
                    style(&tap.name).bold());
                crate::shard::package_processor::add_tap(&tap.name)?;
            }
        }
    }
    ...
    let options = crate::shard::apply::ApplyOptions {
        allow_system_package_removal: false,
        quiet_mode: true,
    };
    
    // Use the version with options to enable quiet mode
    crate::shard::apply::apply_internal_with_options(manifest, false, false, &options)?;
